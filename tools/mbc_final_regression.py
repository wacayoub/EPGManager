#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Final frozen-provider regression gate for MBC/Shahid receiver output."""
...
# Exact source decisions already validated independently. These are frozen so a
# future upstream priority change cannot silently reintroduce the bad guide.
...
def main():
    ...
    for cid in sorted(policy.SECONDARY_REVIEW_IDS | policy.QUARANTINED_IDS, key=str.casefold):
        row = rows.get(cid)
        if row and row.get("auto_lock_safe") is True:
            errors.append("NONCORE_BECAME_AUTOLOCK_SAFE=%s" % cid)
    ...
    # The legacy/foreign IDs may remain published for saved mappings, but they
    # must never silently replace a frozen MENA identity in the policy table.
    notes.append("frozen_core=%d" % len(policy.FROZEN_CORE_IDS))
    notes.append("secondary_review=%d" % len(policy.SECONDARY_REVIEW_IDS))
    notes.append("quarantined=%d" % len(policy.QUARANTINED_IDS))
    ...
    if errors:
        ...
        lines.append("- all frozen MBC/Shahid invariants passed")
