...
Diagnostic only. Protected channel variants (language, genre/feed variant and
channel number) must agree before any candidate can be labelled SAFE.
"""
...
NOISE = {"hd", "sd", "uhd", "fhd", "4k", "tv", "channel", "digital", "mono"}
VARIANT_MAP = {
    ...
    "news": "news", "quran": "quran", "quraan": "quran", "koran": "quran",
    "sunnah": "sunnah", "sunna": "sunnah",
    "kids": "kids", "kid": "kids", "children": "kids",
    "drama": "drama", "cinema": "cinema", "movie": "movies", "movies": "movies",
    "series": "series", "action": "action", "premium": "premium", "extra": "extra",
    "business": "business", "music": "music",
    "kurd": "kurd", "kurdi": "kurd", "kurdish": "kurd",
    "syriac": "syriac", "turkuman": "turkuman", "turkmen": "turkuman",
}
...
def norm(v):
    return " ".join(x for x in raw_words(v) if x not in NOISE)


def variants(row):
    words = raw_words("%s %s" % (row.get("id", ""), row.get("name", "")))
    out = set()
    for w in words:
        if w in VARIANT_MAP:
            out.add(VARIANT_MAP[w])
        elif w.isdigit() and 1 <= int(w) <= 20:
            out.add("num:" + w)
    return out


def probes(row):
    vals = {norm(row.get("name", "")), norm(row.get("id", ""))}
...
def main():
    ...
    for target in foreign:
        ...
        for candidate in live_by_country.get(cc, []):
            ...
            cv = variants(candidate)
            variant_match = (tv == cv)
            score = best_score(target, candidate)
            same_key = bool(tkey and ckey and tkey == ckey and variant_match)
            if same_key:
                score = max(score, 0.995)
            ranked.append((score, same_key, variant_match, as_int(candidate.get("events")), candidate, cv))
        ranked.sort(reverse=True, key=lambda x: (x[2], x[0], x[1], x[3]))
        if ranked:
            score, same_key, variant_match, _events, best, cv = ranked[0]
        else:
            score, same_key, variant_match, best, cv = 0.0, False, False, None, set()

        if best is not None and variant_match and (same_key or score >= 0.94):
    ...
    lines = [
        "FOREIGN ID CANONICAL REPAIR AUDIT V2",
    ...
    for x in out["safe_repairs"]:
        lines.append("- %s | %s -> %s | score=%.3f variants=%s events=%d" % (
            x["target_shard"], x["target_id"], x["canonical_id"], x["score"],
            "/".join(x["target_variants"]) or "base", x["canonical_events"]))
    lines += ["", "REVIEW / UNMATCHED"]
    for x in matches:
        if x["verdict"] == "SAFE_CANONICAL_REPAIR":
            continue
        lines.append("- [%s] %s | %s -> %s | score=%.3f variants=%s/%s" % (
            x["verdict"], x["target_shard"], x["target_id"], x["canonical_id"] or "<none>",
            x["score"], "/".join(x["target_variants"]) or "base",
            "/".join(x["canonical_variants"]) or "base"))
