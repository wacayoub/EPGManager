...
The audited canonical receiver identities remain strict. Coverage duration is
informational only because production already limits the receiver window to 48h;
a shorter real guide is acceptable. Structural, language and identity failures
remain hard blockers. Rotana Clip remains an optional standby secondary.
"""
...
FROZEN_CORE_IDS = {
    ...
    "RotanaCinemaEgypt.eg@SD", "RotanaCinemaKSA.sa@SD", "RotanaClassic.sa@SD",
    "RotanaComedy.sa@SD", "RotanaDrama.sa@SD", "RotanaKhalijia.sa@SD",
}
OFFICIAL_ROTANA_CORE_IDS = {
    "RotanaCinemaEgypt.eg@SD", "RotanaCinemaKSA.sa@SD", "RotanaClassic.sa@SD",
    "RotanaComedy.sa@SD", "RotanaDrama.sa@SD", "RotanaKhalijia.sa@SD",
}
REQUIRED_SECONDARY_IDS = {"Rotana M+ HD.sa", "Rotana Music HD.sa"}
OPTIONAL_SECONDARY_IDS = {"RotanaClip.sa@SD"}
...
# Legacy reference values retained for reports only; never PASS/FAIL blockers.
MIN_COVERAGE_HOURS = 29.5
OFFICIAL_MIN_COVERAGE_HOURS = 26.5
MIN_AR_RATIO = 0.90
...
def profile(cid, name, programmes):
    intervals, titles, descs = [], [], []
    invalid = empty_title = empty_desc = long12 = 0
    ...
    return {
        "id": cid, "name": name, "events": events,
        "coverage_hours": round(coverage, 3), "gaps_gt_2h": gaps,
        "gap_hours": round(gap_hours, 3), "overlaps": overlaps,
        "invalid": invalid, "long_gt_12h": long12,
        ...
        "unique_title_ratio": round(len(title_counter) / float(max(1, events)), 4),
        "top_title_ratio": round(title_counter.most_common(1)[0][1] / float(max(1, events)), 4) if title_counter else 0.0,
    }
...
def core_issues(row):
    issues = []
    reference = required_coverage(row["id"])
    row["minimum_coverage_hours"] = reference
    row["coverage_gate"] = "INFORMATIONAL_ONLY"
    row["coverage_diagnostic"] = (
        "COVERAGE_INFO=%.1fh<legacy-ref-%.1fh" % (row["coverage_hours"], reference)
        if row["coverage_hours"] < reference else ""
    )
    if row["events"] <= 0:
...
def main():
    ...
    for cid in sorted(actual_ids, key=str.casefold):
        ...
        if cid in FROZEN_CORE_IDS:
            ...
            row["class"] = "SECONDARY"; row["secondary_mode"] = "required"
            ...
            row["class"] = "SECONDARY"; row["secondary_mode"] = "optional_standby"
            row["minimum_coverage_hours"] = None
            row["issues"] = secondary_issues(row)
            row["verdict"] = "SECONDARY" if not row["issues"] else "FAIL"
            row["auto_lock_safe"] = False
        else:
            row["class"] = "UNCLASSIFIED"; row["minimum_coverage_hours"] = None
            row["issues"] = ["UNEXPECTED_RECEIVER_ID"]; row["verdict"] = "FAIL"
            row["auto_lock_safe"] = False
        rows.append(row)
        if row["issues"]:
    ...
    required_secondary_ok = sum(1 for r in rows if r.get("class") == "SECONDARY" and r.get("secondary_mode") == "required" and r.get("verdict") == "SECONDARY")
    optional_secondary_present = sum(1 for r in rows if r.get("class") == "SECONDARY" and r.get("secondary_mode") == "optional_standby")
    optional_secondary_ok = sum(1 for r in rows if r.get("class") == "SECONDARY" and r.get("secondary_mode") == "optional_standby" and r.get("verdict") == "SECONDARY")
    status = "FAIL" if errors else "PASS"
    summary = {
        "status": status, "channels": len(rows), "core_expected": len(FROZEN_CORE_IDS),
        "core_ok": core_ok, "required_secondary_expected": len(REQUIRED_SECONDARY_IDS),
    ...
    payload = {
        "schema": 4, "summary": summary, "channels": rows,
        "missing_required_ids": missing_required, "optional_missing_ids": optional_missing,
        "unexpected_ids": unexpected, "errors": errors,
    }
    Path(args.json).write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lines = [
        "ROTANA CANONICAL PROVIDER AUDIT: %s" % status,
        "channels=%d core=%d/%d required_secondary=%d/%d optional_present=%d/%d coverage=informational-only receiver_max=48h" % (
            len(rows), core_ok, len(FROZEN_CORE_IDS), required_secondary_ok, len(REQUIRED_SECONDARY_IDS),
            optional_secondary_present, len(OPTIONAL_SECONDARY_IDS)), "",
    ]
    for row in rows:
        ref = row.get("minimum_coverage_hours")
        ref_text = "n/a" if ref is None else "%.1fh" % ref
        mode = row.get("secondary_mode") or "core"
        lines.append("[%s] %s class=%s mode=%s events=%d coverage=%.1fh legacy_ref=%s gaps=%d title_AR=%.0f%% desc_AR=%.0f%% auto_lock=%s" % (
            row["verdict"], row["id"], row["class"], mode, row["events"], row["coverage_hours"], ref_text,
            row["gaps_gt_2h"], row["title_ar_ratio"] * 100.0, row["desc_ar_ratio"] * 100.0,
            "YES" if row["auto_lock_safe"] else "NO"))
        if row.get("coverage_diagnostic"):
            lines.append("  diagnostic=%s" % row["coverage_diagnostic"])
        if row["issues"]:
