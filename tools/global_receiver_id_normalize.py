...
# Arabic labels that occur in parallel with Latin receiver identities. This is
# deliberately limited to well-known, unambiguous brands; no fuzzy AI deletion.
ARABIC_NAME_EQUIV = {
...
def ascii_words(value: str) -> list[str]:
    x = unicodedata.normalize("NFKD", value or "").encode("ascii", "ignore").decode("ascii")
    x = re.sub(r"^(?:en|ar):\s*", "", x, flags=re.I)
    x = re.sub(r"\b(?:UHD|FHD|HD|SD|DIGITAL|MONO)\b", " ", x, flags=re.I)
...
def old_id_label(cid: str) -> str:
    ...
    s = re.sub(r"\.(?:ae|sa|eg|qa|iq|jo|lb|kw|bh|om|ye|dz|tn|ly|sd|sy|mr|ps|uk|us|net|ir|il|fr|de|nl)$", "", s, flags=re.I)
...
def slug(label: str, cid: str) -> str:
    ...
    # Avoid excessively long IDs while keeping them human-readable.
    s = ".".join(words)
...
def strip_provider_prefix(s: str, ns: str) -> str:
    ...
    while parts and parts[0].casefold() in {low, low + "tv"}:
        parts.pop(0)
    if ns == "MBC" and parts and re.fullmatch(r"MBC\d+", parts[0], flags=re.I):
        parts[0] = parts[0][3:]
...
def canonical_id(stem: str, cid: str, name: str) -> str:
    ...
    if stem in COUNTRIES:
        suffix = COUNTRIES[stem]
        return f"{slug(name, cid)}.{suffix}"
...
def generic_ratio(rows: list[ET.Element]) -> float:
    ...
    generic = sum(1 for x in vals if not x or x in {"program", "programme", "tv program", "news", "beIN sports max".casefold()})
...
def main() -> int:
    ...
    for stem, (_, _, chans, _) in shards.items():
        ...
        for cid, c in sorted(chans.items(), key=lambda kv: kv[0].casefold()):
            candidate = canonical_id(stem, cid, dname(c))
            base_candidate = candidate
            n = 2
            while candidate in used:
                candidate = re.sub(r"(\.[^.]+)$", f".{n}\\1", base_candidate)
                n += 1
            used.add(candidate)
            old_to_new[cid] = candidate
            owner[cid] = (stem, dname(c))
            if candidate != base_candidate:
                collisions.append({"source": stem, "old_id": cid, "new_id": candidate})

    # Conservative proven clone detection across exclusive shards. We require same
    # normalized identity AND byte-for-byte same full timeline, and refuse generic
    # schedules where distinct event channels can legitimately look identical.
    ...
    for members in exact_groups.values():
        ...
        winner = members[0]
        losers = members[1:]
        # Never collapse two services from the same provider shard automatically.
        if any(x[0] == winner[0] and x[0].startswith("provider-") for x in losers):
            continue
        for _, cid in losers:
            drop_ids.add(cid)
        duplicate_groups.append({"canonical_source": winner[0], "canonical_old_id": winner[1], "removed": [x[1] for x in losers]})

    reports = []
    # Apply renames/removals to every XMLTV file, including combined/premium views.
    for path in sorted(base.glob("*.xml.gz")):
        ...
        if txt.exists():
            ...
            txt.write_text("".join(f"{cid}|{names.get(cid,cid)}\n" for cid in kept_channel_order), encoding="utf-8")
    ...
    payload = {
        ...
        "files": reports,
        "mapping": [{"source": owner.get(old, ("", ""))[0], "old_id": old, "new_id": new, "name": owner.get(old, ("", ""))[1]} for old, new in sorted(old_to_new.items())],
    }
    ...
    for stem in EXCLUSIVE:
        rows = [x for x in payload["mapping"] if x["source"] == stem]
        if not rows:
            continue
        lines.append(f"=== {stem} | {len(rows)} IDs ===")
        for r in rows:
            flag = " DROP_DUP" if r["old_id"] in drop_ids else ""
            lines.append(f"{r['old_id']} -> {r['new_id']} | {r['name']}{flag}")
        lines.append("")
    if duplicate_groups:
        lines.append("=== PROVEN DUPLICATES REMOVED ===")
        for g in duplicate_groups:
            lines.append(f"KEEP {g['canonical_source']}::{g['canonical_old_id']} | DROP {' ; '.join(g['removed'])}")
